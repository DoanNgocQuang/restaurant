    ...
    return `
      <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
        <td class="px-6 py-4 whitespace-nowrap">
          <img src="${window.AdminApp.escapeHtml(window.AdminApp.getImageUrl(combo.imageUrl))}" alt="${window.AdminApp.escapeHtml(combo.name)}" class="h-12 w-12 rounded-lg object-cover border border-slate-200 dark:border-slate-700" onerror="this.src='https://placehold.co/600x400?text=No+Image'">
        </td>
        <td class="px-6 py-4">
  ...
  const content = `
    ...
    <div>
      ...
      <div class="flex gap-2">
        ...
        <input type="file" id="combo-image-file" accept="image/*" class="hidden" onchange="uploadComboImage(event)" />
        <button type="button" onclick="document.getElementById('combo-image-file').click()" class="rounded-xl bg-slate-200 px-4 py-2 text-sm font-bold text-slate-700 transition-colors hover:bg-slate-300 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600 whitespace-nowrap">
          Tải lên
        </button>
      </div>
    </div>
...
window.uploadComboImage = async function(event) {
  ...
  try {
    ...
    document.getElementById('combo-image-url').value = payload.data;
